...
<body>
	<div id="write_container">
		<form name="comForm" role="form" method="post" onsubmit="insert(document.comForm); return false">
 			<input type="hidden" id="nav" name="nav" value="${nav}">
 			<input type='hidden' name='postId' id='postId' value="${brdVO.postId}">
 			<input type="hidden" name="group" value="${brdVO.group}">
 			<input type="hidden" name="step" value="${brdVO.step}">
 			<input type="hidden" name="indent" value="${brdVO.indent}"> 
			...
			<div id="editer_board">
				...
				<!-- <label class="item blind" for=boardCategory>게시판</label> -->
					<div class="selIdDiv">
						<select id="selBoard" class="menuId heightMg" name="brdId" readonly>
							<option value="${brdVO.brdId}" selected>${brdVO.brdName}</option>
							<%-- <c:forEach items="${brd}" var="brd">
								<c:if test="${brdVO.brdId eq brd.BRD_IDX}">
...
<!-- 						<button onclick="makeImg()">
					...
					<div id="li_txt">
						...
						<iframe id="content_iframe" scrolling=yes border=0 width="95%" height=300 frameborder=0></iframe>
					</div>
