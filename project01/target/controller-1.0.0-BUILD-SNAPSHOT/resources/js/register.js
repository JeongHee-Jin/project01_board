...
function registerView(){
	...
	//첨부파일 삭제
	 $(".uploadResult").on("click", "button", function(e){
		 ...
		 var type = $(this).data("path"); 
		 alert(targetFile);
		 $.post("/deleteFile",{targetFile},function(arr){
				alert(arr);
			},'json');
		 targetLi.remove();
	 });
	...
	if(postId!=null){
		$.getJSON("/getAttachList",{nav,postId},function(arr){
			...
			$(arr).each(function(i,obj) {
				...
				str += "<button type='button' data-file=\'"+fileCallPath+"\' data-type='file' class='btn_file'>" +
	  ...
	  $(uploadResultArr).each(function(i, obj){
		   //이미지 게시판에 올리기
	        if(obj.image){
	          ...
	          str += "<li data-path = '"+obj.uploadPath+"'><div>";
	          str += "<span> "+ obj.fileName+"</span>";
...
function chageBoard(){
	...
	$.ajax({
		...
		success:function(data){
			$(selBCate).empty().data('options');	
			var option= $(data).find("#selBCate").html();
			selBCate.innerHTML=option;
		},error : function(error) {
			console.log("err",error);
		}		
	})
